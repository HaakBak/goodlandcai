...
const Sidebar = ({ role, historyData = [] }) => {
  ...
  const handleLogout = async () => {
    const now = new Date();
    await addHistoryLog({
      type: role === 'ADMIN' ? 'Admin Logout' : 'Manager Logout',
      description: `${role === 'ADMIN' ? 'Admin' : 'Manager'} logged out of the system`,
      ...
      <aside className="w-72 bg-slate-800 text-white flex flex-col shadow-2xl fixed left-0 top-0 h-screen overflow-y-auto z-10">
        ...
        <div className="p-6 border-t border-slate-700">
          <button 
            ...
            className="flex items-center justify-center w-full space-x-3 text-slate-300 bg-slate-700 py-4 rounded-xl hover:bg-slate-600 transition-all font-bold group"
          >
